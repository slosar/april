...
class TabulatedBAODVLikelihood (BaseLikelihood):
    def __init__(self,name,filename,fid_theory, z):
        ...
        #find minimum
        alphamin=minimize(self.chi2i,1.0,bounds=[[0.9,1.1]],method='SLSQP').x[0]
        rms=quad(lambda x:exp(-self.chi2i(alphamin+x)/2)*x*x,-0.1,0.1)[0]
        rms/=quad(lambda x:exp(-self.chi2i(alphamin+x)/2),-0.1,0.1)[0]
        DVf=self.fidDVOverrd*fid_theory.rd
        print name,"measurement in", fid_theory.rd_approx," : DV=",DVf*alphamin,"+-", sqrt(rms)*DVf
