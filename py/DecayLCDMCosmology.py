...
class DecayLCDMCosmology(LCDMCosmology):
    ...
    def H2_rxrr_a(self,a,rx,rr):
        ...
        return self.Ocb*(1-self.xfrac)/a**3+self.Omrad/a**4+NuContrib+(1.0-self.Om-self.Or)+rx/a**3+rr/a**4
    ...
    def SolveEq(self):
        if (self.lam==0):
            self.Or=0
            yinit= array([self.Ocb*self.xfrac, self.Or])
            ...
            while (abs(eps)>1e-7):
                mid=(lowr+highr)/2
                yinit= array([self.Ocb*self.xfrac, mid])
                sol =odeint(self.RHS,yinit,self.logar)
                self.sol=sol
                eps=sol[-1,1]
                #print "tried: ", mid,eps
                ...
                if (eps>0):
                    ...
                    lowr=mid
                if (c>100):
                    print "Still fucked after 100 tries."
        ...
        self.Ocbh2_early=(self.Ocb*(1-self.xfrac)+sol[-1,0])*self.h**2        
