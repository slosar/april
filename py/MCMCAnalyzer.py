...
class MCMCAnalyzer:
    ...
    def RunChain(self):
        ...
        while not (self.done):
            ...
            if (ploglike>self.cloglike):
                ...
                accept = (exp((ploglike-self.cloglike)/self.temp)>uniform(0.,1.))
    ...
    def ProcessAccepted(self,ppars, ploglike, ploglikes):
        ...
        if (self.co>self.skip):
            #weight rescaled
            wers=self.cw*exp((self.cloglike-self.logofs)*(self.temp-1.0)/self.temp)

            if (self.composite):
                outstr=self.formstr%tuple([wers,-self.cloglike]+vec +self.cloglikes.tolist())
            else:
