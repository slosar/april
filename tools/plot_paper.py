...
for l in lst:
  ...
  if l=='S5' or l=='S5g':
	...
	model   = ['waCDM']
	params  = ['w_z','wa']
	T.Plotting_waDE_2d(dire,model, extra, datasetl, params, legcolor=True, Nb=20)
  ...
  if l=='S5' or l=='S5h':
	...
	model   = ['owaCDM']
	params  = ['w_z','Ok']
	T.Plotting_waDE_2d(dire,model, extra, datasetl, params,loc='upper left',legcolor=True, Nb=25)
...
###################    Extra
  ...
  if l=='smoking' or l=='mnu2':
	...
	params   =  ['mnu']
        ...
        lw=[4,2,4,2,4,2]
	T.Plotting_1d(dire, model, extra, datasetl, params, Nb=40, linecolor=lcolor, lwidth=lw, legend=leg, minmax=[0.0,1.0])
        pylab.xlim(0,1.0)
        pylab.xlabel('$\\sum m_\\nu$')
        pylab.tight_layout()
  ...
  if l=='Early':
	...
	datasetl = ['BBAO+Planck', 'SN+Planck', 'BBAO+SN+Planck']
        model     =  ['SlowRDE']
        params    =  ['dw', 'h']
        T.Plotting_2d(dire, model, extra, datasetl, params, Nb=35,legcolor=True)
        pylab.xlim(-0.5,0.9)
	pylab.ylim(0.55,0.8)
        pylab.tight_layout()
...
################## H plot
  ...
  if l=='HLadder':
	...
	T.plotH(1.6,'','/astro/u/anze/work/Planck/PLA/base/planck_lowl_lowLike/base_planck_lowl_lowLike','g', '$\Lambda$CDM Planck (full)')
	T.plotH(1.8,'','/astro/u/anze/work/Planck/PLA/base/WMAP/base_WMAP','g', '$\Lambda$CDM WMAP (full)')
